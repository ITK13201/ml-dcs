...
class LTSRegressionModel(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.fc2 = torch.nn.Linear(self.HIDDEN_CHANNELS, self.OUTPUT_CHANNELS).to(DEVICE)
...
class GNNDataUtil:
    ...
    def _split_mtsa_results(self, mtsa_results: List[MTSAResult]):
        training_results, testing_results = train_test_split(mtsa_results, test_size=0.2, random_state=DEFAULT_RANDOM_STATE)
    ...
    def __init__(self, mtsa_results: List[MTSAResult], target_name: str):
        ...
        self.training_mtsa_results, self.testing_mtsa_results = self._split_mtsa_results(self.mtsa_results)

    def get_lts_graph(self, lts: List[List[int]], lts_number: int, number_of_states: int, is_environment: bool) -> Data:
        ...
        edge_weight_tensor = torch.tensor(edge_weight, dtype=torch.float).to(DEVICE).to(DEVICE)
        ...
        lts_embedding = Data(x=node_features_tensor, edge_index=edge_index_tensor, edge_weight=edge_weight_tensor).to(DEVICE)
    ...
    def get_lts_set_graph(self, mtsa_result: MTSAResult):
        ...
        for environment in mtsa_result.initial_models.environments:
            lts_embeddings.append(self.get_lts_graph(lts=environment.quantified_structure, lts_number=cnt, number_of_states=environment.number_of_states, is_environment=True))
        for requirement in mtsa_result.initial_models.requirements:
            lts_embeddings.append(self.get_lts_graph(lts=requirement.quantified_structure, lts_number=cnt, number_of_states=requirement.number_of_states, is_environment=False))
