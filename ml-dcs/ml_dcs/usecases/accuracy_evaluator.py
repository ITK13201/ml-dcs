...
class AccuracyEvaluator:
    ...
    def evaluate(self, threshold: float) -> (EvaluationResult, EvaluationResult):
        ...
        for index, actual_value in enumerate(
            self.simple_testing_result_set.result_at_best_accuracy.actual_values
        ):
            if actual_value < threshold:
                updated_predicted_value = self.simple_testing_result_set.result_at_best_accuracy.predicted_values[
                    index
                ]
        ...
        for index, actual_value in enumerate(self.gnn_testing_result.actual_values):
            if actual_value < threshold:
                updated_predicted_value = self.gnn_testing_result.predicted_values[
                    index
                ]
                updated_actual_values.append(actual_value)
                updated_predicted_values.append(updated_predicted_value)
